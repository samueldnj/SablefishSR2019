            ...
            xLim=NULL, yLim=NULL ) )
{
  ...
  if ( gfx$bygears )
  {
    ...
    #for ( i in 1:nGear )
    {
      ...
      mtext( side=1, line=.OUTLINE4, cex=.CEXLAB, outer=TRUE, "Classe d’âge" )
      mtext( side=2, line=2.5, cex=.CEXLAB, outer=TRUE,
             "Proportion selon les classes d’âge" )
      if ( !is.null(seriesNames) )
                     ...
                     doLegend=TRUE, xLim=NULL, yLim=NULL ) )
{
  ...
  if ( gfx$bygears )
  {
    ...
    for ( g in 1:nSeries )
    {
      ...
      if(g==1){indexName ="Casier"}
      if(g==2){indexName ="Std."}
      if(g==3){indexName ="RASC"}
     
      panLab( 0.025,0.8, adj=0, cex=1.4, indexName )      
    ...
    mtext( side=1, line=.OUTLINE1, cex=1.2, outer=TRUE, "Année" )
    mtext( side=2, line=.OUTLINE2, cex=1.2, outer=TRUE, "Indices observés et ajustés (mis à l’échelle) [en milliers de tonnes]" )
  }
  else
  {
    # Y-axis limits.
