...
of the 5 posterior points, the operating model was conditioned on a 
...
estimate of the posterior density at each of the 5 centres as a 
plausibility score for weighting MP performance across the 
5 OMs within each of the reference and robustness sets.
...
recruitment based on the stock-recruitment relationship resulting in an
...
is below 40% of $B_{MSY}$; (iv) __a meta rule__ stating that TAC increases are 
...
that only vary in their at-sea release regulations. The MP variants are 
constructed by combining 3 features: 
...
In this Science Response, MPs are named by concatenating the three at-sea
release settings detailed above: CAP_ALLOCATION_AMORTIZATION. For example, 
...
regulations, below we provide the sequence of calculations used to establish 
...
of releases allcoated to fleet $g$.
...
straightforward – choose the MP with better performance on the fifth criterion. 
Unfortunately, MPs typically differ on all 5 dimensions simultaneously, which 
makes it difficult to compare performance without, at least, establishing some 
equivalency between conservation probabilities (performance dimensions 1-3) and 
short-term average catch (performance dimension 5). 
...
MPs separately to the reference OM and 
...
but less so for females than males. The trawl age fits showed a large 
positive error for age-2 for fish, which appeared to arise from the 2017 and 
...
uncertainty, in the magnitude of the estimated 2015 year-class.
...
estimate of this year-class was about 8 times the historical average (see 
...
#### Reference OM set under reference $F_{2022}$ tuning
...
defined by Objectives 1-3 (Table 2). All tuned MPs were able to meet 
...
while others were projected to just make $B_{MSY}$ by the final year.
The probability of catch falling below the 1,992 t floor (Objective 4) 
was 2% or less across all MPs (Table 2).

Tuning MPs to meet Objectives 1-3, and specifically
treating Objective 3 as a target, focuses MP performance
differences on average annual catch over the next 10 years (Table 2; Objective
...
to 3,700 t per year for regulations with a cap 150% higher than average, 
...
$F=7.4\%$/yr on legal-sized fish (Table 2; $F_{2022}$).

Differences in average annual catch were smaller among at-sea regulation 
options that involved a size limit. A 0% at sea-release cap and 5 year
amortization (MP17) resulted in catches about 400 t higher than the 
...
higher than the current MP, depending on the allocation
and the amortization period (MP3 and MP5 vs. MP15; Table 2). Interestingly,
a 10 year amortization with a 0% cap resulted in a 10-year average catch
that was identical to that of a 50% cap with a historical allocation and 
5 year amortization period (MP 3 vs MP16; Table 2).
...
(MP 10 vs 13; Table 2). Although average 10-year catches were similar, 
...
large (4/5+) legal-sized Sablefish. Indeed, the average revenue per 
tonne was approximately $170 lower for a no size limit trap fishery 
compared to any of the other at-sea 
...
The next best at-sea release regulation, from a total catch and
...
#### Robustness OM set under robustness $F_{2022}$ tuning
...
the conservation objectives [@cox2019evaluating].
...
current MP (MP15, noCap) to 2,767 t under no size limit (MP14, NSL). Thus, 
...
for trawl and next-most-restrictive for trap and longline hook (MP17, 
**cap0_am5**; Table 6).
